...
1. We hypothesize that A) abnormal HRV will associate with obstructive CAD (stenosis > 70%), abnormal HRV will associate with a greater number of obstructed vessels in a dose-response manner
...
compareGroups(data = df, sad ~ HF + LF + VLF + AC + DC + SampEn + ApEn + DYX) %>%
  createTable(.) %>%
...
tmp <- angio_scores
tmp$cad <- ifelse(tmp$cass70 > 1, 1, 0)
